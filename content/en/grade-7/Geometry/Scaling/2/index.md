...
=15/3.75=4
Therefore what can be concluded is that when the dimensions of the rectangle were halved, the area became 1/4 of it. If the length and width of a rectangle are doubled then the area will be 4 times larger than that of the original. 
...
To maintain the shape, scaled figures must have the same angles even though the sides change with the same ratio. We know that the dimensions of the shape on the left are double the shape on the right. Have a look at the images, 5 is double 2.5. Therefore, the side measuring 5 and the side measuring 2.5 units. Similarly, 3 is double of 1.5 and 2 is double of 1. In all the cases the ratio is the same, one side is twice the other. They are corresponding sides. 
...
If the dimensions were scaled by 3 times the area would increase by 9 times and so on (or the square of the scale factor). Try it out with an example and check if it's true. 
...
<img src="5_11_scale_measurement.jpg" width="400" style="display: block; margin: 0 auto;">
5.11
...
Earlier, we said that the dimensions are written as ratios, like the ratio of length to breadth as 10: 8. We can also express the same using a ratio, like 2: 1. 
In 2:1 we are saying that the original is 1 unit and the scaled copy is twice the original for all dimensions. This scaling factor is useful while drawing small objects and the features on it have to be shown in detail.   
If the first part of the ratio is greater than 1 then the scaled figure must be greater than the original figure. If it is smaller than 1 then the scaled figure is smaller than the original figure. 

<img src="5_12_circles_scaled_up.jpg" width="500" style="display: block; margin: 0 auto;">
5.12

The scale factor between the above figures is 2. 


<img src="5_13_triangles_scaled_down.jpg" width="400" style="display: block; margin: 0 auto;">
5.13
