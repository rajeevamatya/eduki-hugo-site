...
description: "Equations are used to relate two quantities that depend on each other. The relationship can be represented by an equation with an equal sign. For example, the price of pens is twice the number of pens. Similarly, the number of candies each student gets is determined by the total number of candies and the number of students. In some cases, the larger quantity is multiplied by a factor to equal the smaller quantity. The way an equation is written doesn't affect the relationship between the two quantities. In the example of a rectangular window, the height is 6 inches less than the width, which can be represented as w-6=h or w=h+6."
